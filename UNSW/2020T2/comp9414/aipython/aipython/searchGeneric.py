...
class Searcher(Displayable):
    ...
    def search(self):
        ...
        while not self.empty_frontier():
            ...
            path,value = self.frontier.pop()
            self.display(2, "Expanding:",path,"(cost:",path.cost,")")
            self.num_expanded += 1
            print("path: ",path,"  path end is:",path.end())
            if self.problem.is_goal(path.end()):    # solution found
                ...
                for arc in reversed(list(neighs)):
                    print("arc : ",arc," /bfs Path:",Path(path,arc))
...
def test(SearchClass, problem=searchProblem.Romania_map, solution=['g','d','c','b','a'] ):
